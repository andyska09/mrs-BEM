...
double mybem::longitudinalFlapping(const PropState& s) {
  std::vector<double> mu(7, 1), Omega(7, 1);
  const double p = s.rate[0];
  const double q = s.rate[1];
  const double vind = s.vind;
  const double alpha = s.alpha;

  for (size_t i = 1; i < mu.size(); ++i) mu[i] = mu[i - 1] * s.mu;
  for (size_t i = 1; i < Omega.size(); ++i)
    Omega[i] = Omega[i - 1] * s.Omega;
