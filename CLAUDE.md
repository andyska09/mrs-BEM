...
- **[notes/](notes/)** — living project notes: `current_goal.md` (objective + fixed NN interface + eval ladder), `pipeline_description.md` (7-step recipe for onboarding a new drone), `bem_parameters.md` (full inventory of every C++ BEM param, where it acts, whether tuning it does anything — groundwork for full-param CMA-ES), `bem_limitations.md` (research notes challenging the paper's BEM assumptions & generalization claim), `data_sources.md` (per-column provenance of the 29-col merged CSV), `results.md` + `cmaes_results.md` (BEM tuning / CMA-ES fit results in paper Table II format, and what each `processed_data/bem-*` folder contains).
...
- **Re-identification / CMA-ES workflow (advisor ask; lives in the `cmaes` C++ tool).** The active re-identifier is `code/simulator/src/CMAES.cpp` → the **`cmaes`** executable (a second CMake target alongside `bem-model`). It reads one flat data CSV (the same 29+ col merged/bem layout, `load()` reads cols 1–23), builds a `Quadcopter`, and via `Quadcopter::load` sweeps a **21-entry `REGISTRY`** ([CMAES.cpp:40](NeuroBEM/code/simulator/src/CMAES.cpp#L40)) of tunable params, each `{default, lo, hi}`, **ordered by optimization priority** — `lift_coefficient, drag_coefficient, hinge_spring_constant, lift_offset, hforce_scale, thrust_scale, pitch, twist, chord_inner, chord_outer, horizontal_drag_coefficient, vertical_drag_coefficient, radius, dx, dy, dz, frontarea_x/y/z`, then a **load-only tail** `num_blades, air_density` (kept at default). `--cma N` frees the **first N** of them (`N=3` = just `cl,cd,k`); minimizes **normalized force MSE**, or force+torque with `--joint` (300 gens, `MAXGEN` constant). Config is **YAML in and out**: `./cmaes data.csv` (report at defaults), `./cmaes data.csv config.yaml` (report at values loaded via `loadConfig` from [config.h](NeuroBEM/code/simulator/include/config.h)), `./cmaes data.csv --cma N [--joint]` (run the fit; prints baseline → CMA-ES → best, writes `best.yaml` + a `convergence.csv` trace). There is **no `--tier`/`key=value`/`--gens` CLI, no `identify` tool, and no `make_subset.py`/`subset_20k.csv`** — those older names in git history were superseded. The `processed_data/` `bem` / `bem-vi-baseline` / `bem-tuned` / `bem-007` / `bem-agi` subfolders are outputs of different tunes (see [notes/results.md](notes/results.md) for the exact cl/cd/k and corrections each uses).
...
./cmaes DATA.csv --cma 3 --joint                  # fit first 3 params (cl,cd,k) on force+torque → best.yaml
```
