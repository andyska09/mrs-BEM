...
**What exists today: both halves.** C++ — two binaries (`mybem-apply`, `mybem-tune`) + the `mybem_sim` library, ~2450 lines under [MyBEM/cpp/](MyBEM/cpp/). Python — the `mybem/` package (~505 lines, **PyTorch**, not TF): [drone.py](MyBEM/mybem/drone.py), [data.py](MyBEM/mybem/data.py), [train.py](MyBEM/mybem/train.py), [eval.py](MyBEM/mybem/eval.py), [nets/](MyBEM/mybem/nets/). Still **not written**: the single `mybem` CLI of DESIGN §5 (run modules directly, `python -m mybem.train`) and the sweep/run-table of §4.6/§7.7. DESIGN's status table also lists a `scripts/apply.sh` that does not exist — the only scripts are [parse_flights.py](MyBEM/scripts/parse_flights.py) and [metacentrum/](MyBEM/scripts/metacentrum/). The `@hash6` of DESIGN §2.2 exists for nets ([train.py:85-86](MyBEM/mybem/train.py#L85)) and for cluster tune runs ([submit.py:23-26](MyBEM/scripts/metacentrum/submit.py#L23)); `store/preds/<name>/` is still flat, since a PREDS folder is named by the model's `name:` field. `Component::diagnose()`/`diagnostics()` ([component.h:21-22](MyBEM/cpp/include/mybem/component.h#L21)) are declared and never called. **`MyBEM/DESIGN.md` is still untracked** (`??` in git status).
