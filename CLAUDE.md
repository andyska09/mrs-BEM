...
- **Re-identification / CMA-ES workflow (advisor ask; lives in the `cmaes` C++ tool).** The active re-identifier is `code/simulator/src/CMAES.cpp` → the **`cmaes`** executable (a second CMake target alongside `bem-model`). It reads one flat data CSV (the same 29+ col merged/bem layout, `load()` reads cols 1–23), builds a `Quadcopter`, and via `setParams` (above) sweeps a 16-entry `REGISTRY` of tunable params — `cl, cd, k, theta0, theta1, ci, co, cl_offset, hforce_scale, thrust_scale, dx, dy, dz, Ax, Ay, Az`, each with `{default, lo, hi}` bounds. `--tier 0..3` frees the first `{3,7,10,16}` of them (tier 0 = just `cl,cd,k`); it minimizes **normalized force MSE**, or force+torque with `--joint`. CLI: `./cmaes data.csv` (report at defaults), `./cmaes data.csv cl=14.3 cd=14.5` (report at given values), `./cmaes data.csv --cma [--tier N] [--joint] [--gens N]` (run the fit; prints baseline → CMA-ES → best). There is **no `identify` tool and no `make_subset.py`/`subset_20k.csv`** — those older names in git history were superseded by `cmaes` reading a data CSV directly. The `processed_data/` `bem`, `bem-vi-baseline`, `bem-bad-tune` subfolders are outputs of different tunes.
...
./cmaes DATA.csv --cma --tier 0 --joint --gens 300  # fit cl,cd,k on force+torque
```
