...
- **[notes/](notes/)** — living project notes: `paper_pipeline_description.md` (7-step recipe for onboarding a new drone + fixed NN interface), `agi_notes.md` (BEM tuning results in paper Table II format, what each `processed_data/` tune contains, and the four empirical corrections the agilicious BEM adds over the paper BEM), `bem_parameters.md` (full inventory of every C++ BEM param, where it acts, whether tuning it does anything — groundwork for full-param CMA-ES; includes the canonical **REGISTRY bit ↔ code-variable** table that decodes `--cma MASK`), `bem_limitations.md` (research notes challenging the paper's BEM assumptions & generalization claim), `data_sources.md` (per-column provenance of the 29-col merged CSV).
...
- **NN-vs-paper comparison (current step):** [generate_ablation_study.py](NeuroBEM/code/Python/generate_ablation_study.py) loads a `train_logs/<ts>/best_model` and runs it over `data/bem/test`, printing per-axis force/torque RMSE (compare to paper Table I/II) and writing per-signal `.png`/`.csv` to `--output_dir` (used as `eval_out/`).
- **Deployment path** (trained `.pb` → ONNX): the model is exported to `best_model/network.onnx` (via `tf2onnx`); [verify_onnx.py](NeuroBEM/code/Python/verify_onnx.py) checks ONNX == TF on a ones-input `(1,20,10)`. Agilicious runs it via **ONNX Runtime**; a TensorRT path also exists (`.trt` engine, C++ check under [NeuroBEM/code/Python/trt/](NeuroBEM/code/Python/trt/)). ONNX/engines are machine-specific and gitignored.
