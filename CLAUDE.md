...
- **[research/](research/)** — reading material and write-ups. Two folders, nothing else: no index, no schema, no maintenance skills. Conceptual questions are answered from `sources/` + the code with a `file:line` citation.
  ...
  - **[research/reports/](research/reports/)** — findings: concise, information-dense, no fluff. **Write one only when I ask** — never file a report unprompted. `CMAES-identification.md` and `Generalization.md` are still empty placeholders; [Architectures.md](research/reports/Architectures.md) is at "screening done (seed 0, n=1), 3-seed finals pending" and [Feature-ablation.md](research/reports/Feature-ablation.md) is a written-up design at "proposed, not run". **A report may exist before its numbers do, and a report with numbers may still be n=1 — read the Status line before quoting one.**
...
```
Apply a model (`MODEL.yaml INPUT PREDSDIR`; INPUT is one `merged_*_seg_*.csv` or a folder of them, output goes to `PREDSDIR/<name>@<hash>/<id>.csv` + `params.yaml`, existing outputs are skipped so re-running resumes — **but only within the same hash, so touching the config starts a fresh folder**, `OMP_NUM_THREADS` segments at a time):
```
