...
- **CMA-ES tuning:** re-fitting the BEM aero params `(cl, cd, kβ)` from flight data (see "re-identification" below) is now an explicit advisor ask — evaluate what CMA-ES can recover before touching the NN. The `processed_data/` `bem`, `bem-vi-baseline`, `bem-bad-tune` subfolders are outputs of different tunes.
...
- **[notes/](notes/)** — living project notes: `current_goal.md` (objective + fixed NN interface + eval ladder), `pipeline_description.md` (7-step recipe for onboarding a new drone).
...
2. **Flight-data processing (MATLAB) → base-model application (C++)** — Merge sensor sources, then run the BEM over them to produce training data.
   ...
   - The C++ `bem-model` executable ([NeuroBEM/code/simulator/](NeuroBEM/code/simulator/)) reads each `merged_*_seg_*.csv`, predicts forces/torques, and writes `MODEL/MODEL_<flight>_seg_X.csv` (the NN training data = measured minus predicted). Run via `Scripts/applyBM.sh DATAFOLDER`.
...
- **Aero params are compile-time constants.** The propeller `param` is `static constexpr Propeller_s` ([NeuroBEM/code/simulator/include/propeller.h:82](NeuroBEM/code/simulator/include/propeller.h#L82)) with the `params.h` defaults as the single source of truth; changing `cl`/`cd`/`k` needs a rebuild. (An earlier experiment made `param` an instance member with a `setAero()` runtime override so the `identify` tool could re-fit without recompiling; that was reverted in the working tree along with `identify.cpp` — see the re-identification note below.)
...
- [NeuroBEM/analysis/utils.py](NeuroBEM/analysis/utils.py) — shared loaders, noise metrics, and the **canonical column layout**. `load_flight`/`load_largest_segment` read `processed_data/merged_*_seg_*.csv` (29-column merged order hard-coded in `COLUMNS`). `load_bem_flight`/`load_bem` read `processed_data/bem/bem_*_seg_*.csv`. Files on disk are 47 columns historically (29 merged + 6 predicted `fx,fy,fz,tx,ty,tz` cols 29–34 + 12 per-motor diagnostics `vi,mu,as` cols 35–46). ⚠ Those 12 diagnostics came from an earlier simulator build; the **current `bem-model` emits only the 6 predicted cols (35 total)**, so regenerating `processed_data/bem/` will break `load_bem`'s `VI`/`MU`/`AS` indexing until the diagnostics path is restored. `measured()`/`residuals()` compute force `= mass·acc` and torque `= I·ang_acc + ω×Iω` using the **simulator** mass/inertia. `FS = 400 Hz`. SNR uses a 4th-order **25 Hz Butterworth** low-pass.
...
- **Re-identification / CMA-ES workflow (advisor ask; the C++ side was removed).** Historically `make_subset.py` pooled all non-test `bem_*.csv`, binned rows on a `(mu, vi)` grid, and sampled ~equally per cell (writing `subset_20k.csv`) so aggressive regimes weren't drowned out by hover; a C++ `identify` tool then ran **CMA-ES** to re-fit `(cl, cd, k)` in normalized coords, minimizing combined force+torque RMSE. Both `make_subset.py`/`subset_20k.csv` and `code/simulator/src/identify.cpp` (plus its `setAero`/`getDiagnostics` plumbing) have been **removed from the working tree** — restore them from git history to re-run. This CMA-ES tuning is exactly what the advisor wants exercised.
...
```
CMake options (default ON): `ENABLE_FAST`, `UNSAFE_MATH`, `ENABLE_PARALLEL` (OpenMP), `EIGEN_FROM_SYSTEM`. C++17, `-march=native`. One target: `bem-model` (pipeline stage 2). The removed `identify` target (CMA-ES aero re-identification) built here before its `identify.cpp` was deleted.
