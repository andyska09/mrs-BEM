...
static const std::map<std::string, std::vector<const char*>> SECTIONS = {
    {"bem",
     {"lift_coefficient", "drag_coefficient", "hinge_spring_constant", "pitch",
      "twist", "chord_inner", "chord_outer", "lift_offset", "hforce_scale"}},
...
static std::map<std::string, double> defaults() {
  ...
  for (const ParamSpec& p : REGISTRY) m[p.key] = p.def;
  return m;
}
...
static void writeYaml(std::ostream& os, const std::map<std::string, double>& c) {
  ...
  for (const auto& [section, keys] : SECTIONS) {
    os << section << ":\n";
    for (const char* key : keys) os << "  " << key << ": " << c.at(key) << '\n';
  }
}
...
static std::vector<Sample> load(const std::string& path) {
  ...
  while (std::getline(file, line)) {
    if (!line.empty() && !std::isdigit(line[0]) && line[0] != '-') continue;
...
static std::map<std::string, double> cma(Quadcopter& quad,
  ...
  auto toConfig = [&](const VectorXd& x) {
    ...
    for (int i = 0; i < N; ++i) c[REGISTRY[i].key] = def[i] + x[i] * scale[i];
    return c;
  };
  ...
  log << "gen,loss";
  for (int i = 0; i < N; ++i) log << ',' << REGISTRY[i].key;
  log << '\n';
  ...
  for (int gen = 0; gen < MAXGEN; ++gen) {
    ...
    for (int k = 0; k < lambda; ++k) {
      VectorXd z(N);
      for (int i = 0; i < N; ++i) z[i] = randn(rng);
    ...
    log << gen << ',' << best_f;
    for (int i = 0; i < N; ++i) log << ',' << def[i] + best_x[i] * scale[i];
    log << '\n';
    if (gen % 10 == 0)
